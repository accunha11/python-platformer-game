...
def level1(window):
    ...
    fire = Fire(BLOCK_SIZE*6.35, FIRE_HEIGHT_OFFSET - BLOCK_SIZE + (BLOCK_SIZE * 2) // 3, 16, 32)
    fire.on()

    box = Box(BLOCK_SIZE*6.25, HEIGHT - BLOCK_SIZE * 2.85, BOX_WIDTH, BOX_HEIGHT)
    hidden_fruit = Fruit(BLOCK_SIZE*6.25 + HIDDEN_FRUIT_WIDTH_OFFSET, HIDDEN_FRUIT_HEIGHT_OFFSET - BLOCK_SIZE * 2.85, FRUIT_SIZE, "Strawberry", False)

    flag = Flag(BLOCK_SIZE*20, FLAG_HEIGHT_OFFSET - BLOCK_SIZE * 5, FLAG_SIZE)
    fruits = [Fruit(BLOCK_SIZE* 9 + 15, FRUIT_HEIGHT_OFFSET - BLOCK_SIZE * 4, FRUIT_SIZE, "Strawberry"),
              Fruit(BLOCK_SIZE* 10 + 15, FRUIT_HEIGHT_OFFSET - BLOCK_SIZE * 4, FRUIT_SIZE, "Strawberry"),
              Fruit(BLOCK_SIZE* 15 + 15, FRUIT_HEIGHT_OFFSET - BLOCK_SIZE * 4, FRUIT_SIZE, "Strawberry"),
              Fruit(BLOCK_SIZE* 16 + 15, FRUIT_HEIGHT_OFFSET - BLOCK_SIZE * 4, FRUIT_SIZE, "Strawberry"),]
