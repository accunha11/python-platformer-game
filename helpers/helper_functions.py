...
def get_background(name):
    ...
    _, _, width, height = (
        image.get_rect()
    )  # underscores are there because I don't need the other two values
    tiles = []
    ...
    for i in range(WIDTH // width + 1):
        for j in range(HEIGHT // height + 1):
            pos = (
                i * width,
                j * height,
            )  # denotes the position of the top left hand corner of the current tile
...
def draw(window, background, bg_image, players, objects, offset_x=0, scoring=[]):
    for (
        ...
        background
    ):  # looping through every tile we have and drawing background image at that posiiton
        ...
        if pygame.sprite.collide_mask(player, obj):
            if not obj.pass_through:
                if dy > 0:
                    ...
                    player.rect.top = (
                        obj.rect.bottom
                    )  # makes sure we don't go through the objects
                    player.hit_head()
                    if obj.name == "box":
                        obj.make_hit()
